      ...
      <v-card class="border">
        ...
        <v-row class="">
          <v-col cols="12" md="3" sm="6" xs="12" class="">
            ...
            <!-- UP TOPS -->
            <h2>
              <Badge label="Tops" :number="projectTops.length" />
            </h2>

            <div v-for="(task, i) in projectTops" :key="`tops_${i}`" no-gutters>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      projectTops: state => state.projects.projectTops
    }),
    ...mapGetters('projects', [
      'unstartedTasks',
      'startedTasks',
  ...
  methods: {
    ...mapActions('projects', ['addNewTask', 'updateTaskColumn', 'deleteTaskAction', 'actionFetchTops']),
    ...
    loadData(){
      this.actionFetchTops({ id: this.selectedBoard })
    }
  },
}
</script>
